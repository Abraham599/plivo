...
async def get_service_current_uptime(service_id: str):
    ...
    try:
        ...
        print(f"Fetching uptime for service: {service_id}")
        ...
        print(f"Fetching checks from {start_date} to {end_date}")
        ...
        try:
            ...
            print(f"Error querying uptime checks: {str(e)}")
            raise HTTPException(
                status_code=500,
                detail=f"Error querying uptime checks: {str(e)}"
            )
        
        if not checks:
            print("No checks found in the last 24 hours")
            return {
                "uptime": None,
                "checksCount": 0,
                "lastCheck": None,
        ...
        # Debug: Print first check's structure
        if checks:
            print(f"First check structure: {dir(checks[0])}")
            print(f"First check status: {getattr(checks[0], 'status', 'status not found')}")
        
        total_checks = len(checks)
        up_checks = sum(1 for check in checks if getattr(check, 'status', '').lower() == "up")
        ...
        result = {
            ...
            "lastCheck": checks[-1].timestamp.isoformat() if checks and hasattr(checks[-1], 'timestamp') else None,
            "currentStatus": getattr(checks[-1], 'status', None) if checks else None
        }
        
        print(f"Returning uptime data: {result}")
        return result
        ...
        print(f"Unexpected error in get_service_current_uptime: {error_trace}")
        raise HTTPException(
            status_code=500,
